...
/// Procesador de corrección de iluminación para iOS
/// Usa algoritmos conservadores para evitar sobre-exposición
class IOSLightingProcessor {
  ...
  static Future<File> correctLighting({
    ...
    try {
      print('[iOSLighting] Starting correction - brightness: ${analysis.averageBrightness.toStringAsFixed(3)}, state: ${analysis.state}');
      
      // Leer y decodificar imagen
      final bytes = await imageFile.readAsBytes();
      final image = img.decodeImage(bytes);
      if (image == null) {
        print('[iOSLighting] Failed to decode image');
        return imageFile;
      }
      ...
      if (brightness < LightingCorrectionConfig.safeMinBrightness ||
        ...
        print('[iOSLighting] Brightness out of safe range, skipping correction');
        return imageFile;
      }
      ...
      if (analysis.state == LightingState.tooDark) {
        ...
        print('[iOSLighting] State is ${analysis.state}, no correction needed');
        return imageFile;
      }
      ...
      print('[iOSLighting] Correction applied successfully: ${correctedFile.path}');
      return correctedFile;
      
    } catch (e, stack) {
      print('[iOSLighting] Error during correction: $e');
      print('[iOSLighting] Stack: $stack');
      return imageFile;
    }
  }
  ...
  static img.Image _brightenImage(img.Image image, LightingAnalysis analysis) {
    ...
    print('[iOSLighting] Brightening - delta: ${safeDelta.toStringAsFixed(3)}, adjust: $brightnessAdjust, gamma: $gamma');
  ...
  static img.Image _darkenImage(img.Image image, LightingAnalysis analysis) {
    ...
    print('[iOSLighting] Darkening - delta: ${safeDelta.toStringAsFixed(3)}, adjust: $brightnessAdjust, gamma: $gamma');
